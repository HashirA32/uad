...
const Counter = () => {
  ...
  useEffect(() => {
        ...
        if (entry.isIntersecting) {
          ...
          const interval = setInterval(() => {
            setCounters(prevCounters => {
              ...
              updatedCounters[counterIndex].count += increment;
              if (updatedCounters[counterIndex].count >= targetNumber) {
                clearInterval(interval);
                updatedCounters[counterIndex].count = targetNumber;
              }
    ...
    document.querySelectorAll('.counter-element').forEach(element => observer.observe(element));
